...
function truncateHtml(html: string, maxLength: number = 900): string {
  ...
  async function fetchOgImageAndSave(url: string) {
    try {
      ...
      try {
        ...
        console.warn(
          'Failed to fetch HTML with net.request, falling back to BrowserWindow:',
          netError
        )
  ...
  ipcMain.handle('create-bookmark-with-screenshot', async (_, url?: string) => {
    try {
      ...
      if (!folder) {
        ...
        mainWindow.webContents.send('bookmark-changed')
        return {
          message: 'bookmark created successfully!!! without connecting to any folder'
        }
      }
